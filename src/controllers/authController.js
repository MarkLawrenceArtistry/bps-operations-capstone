...
const login = async (req, res) => {
    try {
        ...
        await run(`
            ...
            VALUES (?, ?, ?, ?)    
        `, [user.id, "LOGIN", "users", "User logged in successfully"])
...
const createUser = async (req, res) => {
    try {
        ...
        await logAudit(req.user.id, 'CREATE', 'users', result.lastID, `Created user ${username}`);
...
const updateUser = async (req, res) => {
    try {
        ...
        await logAudit(req.user.id, 'UPDATE', 'users', id, `Updated profile for user ID:${id}`);
...
const deleteUser = async (req, res) => {
    try {
        ...
        await logAudit(req.user.id, 'UPDATE', 'users', id, `Disabled user ID: ${id}`);
